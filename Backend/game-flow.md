...
The freeze operation should calculate the final scores and prize results once, using only collection records and phishing records whose event timestamps are less than or equal to `scoring_cutoff_at`, then store a per-user result snapshot including the collection count, phishing count, and scoring constants used in each score. The coordinator publishes this final snapshot before frozen reads become available. This snapshot should be used by `GET /scoreboard`, `GET /scoreboard/me`, and `GET /users/me/prize` while the scoreboard state is `FROZEN`; do not recalculate scoreboard or prize results on every lookup.
